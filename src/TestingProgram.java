        ...
        footballBoard.addMouseListener(new MouseAdapter() {
            ...
            public void mouseReleased(MouseEvent e) {
                ...
                if (e.getX() == oldX && e.getY() == oldY) {
                    ...
                    newPosition = new Point((double) (e.getX() - FootballBoard.TOTAL_SHIFT_X) / FootballBoard.K,
                            (double) (e.getY() - FootballBoard.TOTAL_SHIFT_Y) / FootballBoard.K);
                } else {
                    footballBoard.drawArea(e.getX(), e.getY(), oldX, oldY);
                    newPosition = new ReachabilityArea((double) (oldX - FootballBoard.TOTAL_SHIFT_X) / FootballBoard.K,
                            (double) (oldY - FootballBoard.TOTAL_SHIFT_Y) / FootballBoard.K,
                            Math.sqrt(Math.pow(e.getX() - oldX, 2) + Math.pow(e.getY() - oldY, 2)) / FootballBoard.K,
                            Math.atan2(e.getY() - oldY, e.getX() - oldX) / Math.PI * 180);
                }
...
class FootballBoard extends JPanel {
    ...
    void drawArea(int x, int y, int oldX, int oldY) {
        positions.add(new ReachabilityArea(oldX - START_X, oldY - START_Y,
                Math.sqrt(Math.pow(x - oldX, 2) + Math.pow(y - oldY, 2)),
                Math.atan2(y - oldY, x - oldX) / Math.PI * 180));
        repaint();
    }
    ...
    protected void paintComponent(Graphics g) {
        ...
        for (Position position : positions) {
            if (position instanceof ReachabilityArea) {
                ...
                g.drawOval((int) (area.getXCenter() - area.getRadius()), (int) (area.getYCenter() - area.getRadius()),
                        (int) area.getRadius() * 2, (int) area.getRadius() * 2);
                g.fillOval((int) area.getXCenter() - 2, (int) area.getYCenter() - 2, 4, 4);
                g.drawLine((int) position.getX(), (int) position.getY(), (int) area.getXCenter(), (int) area.getYCenter());
            }
            g.fillOval((int) position.getX() - 4, (int) position.getY() - 4, 8, 8);
